...
class _EditPageState extends State<EditPage> {
  ...
  Future<void> _addToContacts() async {
    ...
    if ((card.primaryName ?? '').trim().isNotEmpty) {
      contact.name = Name(first: card.primaryName!.trim());
    }

    if ((card.primaryCompany ?? '').trim().isNotEmpty ||
        (card.primaryTitle ?? '').trim().isNotEmpty ||
        (card.primaryDepartment ?? '').trim().isNotEmpty) {
      contact.organizations = [
        Organization(
          company: (card.primaryCompany ?? '').trim(),
          title: (card.primaryTitle ?? '').trim(),
          department: (card.primaryDepartment ?? '').trim(),
        ),
      ];
    }
    ...
    contact.websites = [
      for (final u in card.urls) Website(u, label: WebsiteLabel.work),
    ];

    if ((card.primaryAddressLine ?? '').trim().isNotEmpty ||
        (card.primaryPostalCode ?? '').trim().isNotEmpty) {
      contact.addresses = [
        Address(
          '',
          street: (card.primaryAddressLine ?? '').trim(),
          postalCode: (card.primaryPostalCode ?? '').trim(),
